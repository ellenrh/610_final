...
happiness_by_family_savings %>%
  ...
  labs(title = "Figure 1", 
       subtitle = "Happiness levels by family savings",
...
```{r happiness by religion}

# NOTE TO SELF: some examples, need to iron out wrinkles in these. Not sure how to make pivot_longer and pivot_wider work here. See error message.

subset_JW_2 <- clean_data %>%
  select(feeling_of_happiness, religious_person, member_religion)

subset_JW_2 %>%
pivot_longer(
    cols = c(member_religion, religious_person),
    names_to = "Religious status",
    values_to = "Level of observance"
  )

subset_JW_2 %>%
  pivot_wider(
    names_from = "religious_person",
    values_from = "feeling_of_happiness"
  )
