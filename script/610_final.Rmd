...
# running both of these shows that this recoding worked for the "feeling of happiness" variable but not for marital status (or any of the others). Where is the error?
# I realized that if I did them each separately, overwriting data_1 each time, it worked! I also realized that we needed to use unique() to check the exact labels for each variable. They differed a little, so copying and pasting led to some errors. I cleaned these up, it all works now! data_1 has all of the variable levels recoded to "Other" as appropriate.
...
```{r summary statistics, some examples of happiness by group}
